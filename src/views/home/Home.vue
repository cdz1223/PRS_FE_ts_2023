<template>
  <div class="scroll-y">
    ...
    <div class="rowBC" style="margin-top: 10px">
      ...
      <el-button v-roles-permission="['ADMIN']" type="danger" @click="adminNewEditModalShow = true">
        Admin new
      </el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column v-roles-permission="['ADMIN']" align="center" label="Admin" min-width="150">
        <template #default="scope">
          <el-button size="small" type="danger" @click="handleAdminEdit(scope.$index, scope.row)">Admin edit</el-button>
        </template>
      </el-table-column>
      <el-table-column
    ...
    />
    <adminDialog
      v-if="adminEditModalShow && adminEditRow"
      :edit-row="adminEditRow"
      @close="adminEditModalShow = false"
      @refresh="fetchData()"
    />
    <timelineDialog
    ...
    />
    <adminNewDialog v-if="adminNewEditModalShow" @close="adminNewEditModalShow = false" @refresh="fetchData()" />
  </div>
</template>
...
import adminNewDialog from './components/adminNewDialog.vue'
...
const adminEditDialogInfo = reactive<{ adminEditRow: null | RowData; adminEditModalShow: boolean }>({
  adminEditRow: null,
  adminEditModalShow: false
})
const { adminEditRow, adminEditModalShow } = toRefs(adminEditDialogInfo)
const handleAdminEdit = (index, row: RowData) => {
  adminEditModalShow.value = true
  adminEditRow.value = row
}
const adminNewEditDialogInfo = reactive<{ adminNewEditModalShow: boolean }>({
  adminNewEditModalShow: false
})
const { adminNewEditModalShow } = toRefs(adminNewEditDialogInfo)
